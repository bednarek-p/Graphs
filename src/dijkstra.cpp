...
{
    time.start();
   for(int i=0;i<Matrix_graph.size()-1;i++)
    {
        int u = minimal_distance(Matrix_graph,dist,is_shortest_way);
        is_shortest_way[u] =true;
        for(int v=0; v<Matrix_graph.size();v++)
        {
            if(!is_shortest_way[v] && Matrix_graph.get_value(u,v) && dist[u]!= INT_MAX && dist[u]+Matrix_graph.get_value(u,v)<dist[v])
            {
                dist[v]=dist[u]+Matrix_graph.get_value(u,v);
                path_data[v] = u;
            }
        }
    }
...
{
    int* distance= new int [Matrix_graph.size()];
    bool* is_shortest_way= new bool [Matrix_graph.size()];
    int* path= new int[Matrix_graph.size()];
    ...
    Timer time;
    dijkstra_agorithm(Matrix_graph,distance,is_shortest_way,path,time);
    time.print_time_duration();

    append_data_to_file("Matrix_timers_dijkstra.txt",std::to_string(time.return_time_duration())+"\n");
    print_data_to_file("MATRIX/"+std::to_string(density)+"/results_"+std::to_string(test_variation)+"_"+std::to_string(test_number)+".txt",data_to_string(Matrix_graph,distance,path));
